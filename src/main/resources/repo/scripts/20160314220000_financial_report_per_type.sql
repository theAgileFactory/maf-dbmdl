...
group by `pe`.`id`,`wo`.`currency_code`,`wo`.`is_opex`)
...
group by `pe`.`id`,`poli`.`currency_code`,`poli`.`is_opex`)
...
from (`work_order` `wo` join `portfolio_entry` `pe` on((`wo`.`portfolio_entry_id` = `pe`.`id`))
	left outer join `portfolio_entry_budget_line` `pebl` on ((`wo`.`portfolio_entry_budget_line_id` = `pebl`.`id`))
	left outer join `portfolio_entry_budget_line_type` `peblt` on ((`pebl`.`portfolio_entry_budget_line_type_id` = `peblt`.`id`)))
where ((`wo`.`is_engaged` = 1) and (`wo`.`deleted` = 0)) group by `pe`.`id`,`wo`.`currency_code`,`wo`.`is_opex`);
...
INSERT INTO `reporting_authorization` (`expression`, `type`, `deleted`, `last_update`) 
  VALUES ('report.financial_per_type', '0', '0', NOW());
